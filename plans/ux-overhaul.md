...
- [~] **Listen-Editoren vereinheitlicht (Verhaltensebene)** *(PR)*: Geteilte `common/listItemValidation.getDuplicateKeyError` (unit-getestet) für beide Listen-Editoren; SingleSelection nutzt sie jetzt (statt eigener Logik), ChipGroup erhält damit erstmals Eindeutigkeits-/Leer-Prüfung der Chip-`field_id` (Fehler am Feld + „Speichern" gesperrt). Zusätzlich: Chip-Icon über `IconField`-Picker (statt rohem Textfeld) und Leerzustand-Hinweis — konsistente Affordanzen. **Bewusst offen:** Die unterschiedlichen Editiermodelle (Inline-Tabelle für einfache Optionen vs. Card+Dialog für reichhaltige Chips) bleiben, da die Datenformen stark abweichen; ein struktureller Voll-Merge (inline für beide) wäre ein größerer Folge-Schritt.
